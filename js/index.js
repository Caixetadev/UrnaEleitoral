  ...
  if(candidato.length > 0) {
    ...
    for(let i in  candidato.fotos) {
      if(candidato.fotos[i].small) {
        ...
        fotosHtml += `<div class="divisao-1-img small">
      <img
        src="${candidato.fotos[i].url}"
        alt="">
      ${candidato.fotos[i].legenda}
    </div>`

      } else {
      fotosHtml += `<div class="divisao-1-img">
      <img
        src="${candidato.fotos[i].url}"
        alt="">
      ${candidato.fotos[i].legenda}
...
function branco() {
  if(numero === '') {
    ...
    aviso.style.display = 'block';
    numeros.innerHTML = '';
    descricao.innerHTML = '<div class="aviso-grande2 pisca">VOTO EM BRANCO</div>';
  } else {
